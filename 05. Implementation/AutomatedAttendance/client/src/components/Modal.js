...
const CustomModal = ({ visible, onClose, title, fields, onSave, renderField }) => {
  ...
  const renderFieldInput = (field) => {
    ...
    if (renderField) {
      return (
        <View style={field.type === 'instructor-search' ? styles.instructorFieldContainer : null}>
          {renderField(
            field,
            formData[field.key],
...
const styles = StyleSheet.create({
  ...
  fieldContainer: {
    marginBottom: 25, // Increased from 20 to 25 for more spacing between fields
  },
  instructorFieldContainer: {
    marginBottom: 15, // Additional margin for instructor field
